...
import { FaCheckCircle, FaImage, FaHashtag, FaGift, FaBullseye, FaCalendarAlt } from 'react-icons/fa';

export default function BrowseApprovedDonationRequest() {
    ...
    const fetchApprovedRequests = async () => {
        try {
            setLoading(true);
            const response = await getDonationRequestsByUserAndStatus("APPROVED");
            ...
            if (err.response?.status === 404) {
                // Treat 404 as no approved requests available
                setRequests([]);
                setError(null);
                return;
            }
    ...
    if (loading) return <div style={styles.loadingContainer}><p>Loading approved requests...</p></div>;
    ...
    return (
        <div style={styles.container}>
            <div style={styles.header}>
                ...
                <p style={styles.subtitle}>View all your approved donation requests</p>
            </div>
            
            {requests.length === 0 ? (
                <div style={styles.emptyContainer}>
                    <p style={styles.emptyText}>No approved requests found.</p>
                </div>
            ) : (
